...
OUT = REPO / "resultados" / "2026-08-22-trader-ia-empresa"
...
def main() -> None:
    ...
    zip_dir(
        "STLabs-trader-ia-empresa.zip",
        png_retina + [html, B / "caption.txt", B / "MANIFIESTO-FUENTES.md"],
        B,
    )
    zip_dir("STLabs-trader-FHD.zip", png_fhd, B)
    zip_dir("STLabs-trader-4K.zip", png_4k, B)

    if OUT.exists():
        shutil.rmtree(OUT)
    shutil.copytree(B, OUT, ignore=shutil.ignore_patterns("*.pyc", "__pycache__"))

    registrar_carrusel(B, meta)

    artifacts = Path("/opt/cursor/artifacts")
    artifacts.mkdir(parents=True, exist_ok=True)
    shutil.copy(B / "_preview-tira.png", artifacts / "trader-ia-empresa-tira.png")
    for i in (1, 8):
        shutil.copy(B / "png" / f"slide-{i:02d}.png", artifacts / f"trader-ia-slide-{i:02d}.png")
