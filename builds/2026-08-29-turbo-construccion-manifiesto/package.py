...
WORD = REPO / "Word"
ARTIFACTS = Path("/opt/cursor/artifacts/carrusel-turbo-construccion-manifiesto")
OUT_NAME = "STLabs-Turbo-Construccion-Manifiesto"
...
def main() -> None:
    ...
    (WORD / "LEEME.txt").write_text(
        "\n".join(
            [
                "STLabs — Cómo construí Turbo (manifiesto/lino)",
                "Blueprint 4 pasos → sebastian.stlabs.ar",
                f"Slides: {slides} · Keyword: {meta['keyword_portada']}",
                "",
                "Resoluciones:",
                "  slide-XX.png + 4k/  → 4320×5400 PNG (4K)",
                "  fhd/slide-XX.png    → 1920×2400 PNG (Full HD)",
                "",
                "Accent #C9562F · Fondo lino_tela · Solo español",
                "",
            ]
        ),
        encoding="utf-8",
